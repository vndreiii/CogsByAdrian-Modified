...
class Killer(commands.Cog):
    ...
    # SHLAP
    @commands.command()
    async def shlap(self, ctx, *, user: discord.Member):
        ...
        if target['id'] == bot['id']:  # trying to schlap the bot, eh?

            message1 = "{} looks at {}... ✋😏 🤖".format(
                killer['nick'], bot['nick'])
            message2 = "but {} suddenly shlaps {} with his silver sword! 😵💫 🍆🤖".format(
                bot['nick'], killer['nick'])

        elif killer['id'] == target['id']:  # wants to shlap themselves
        ...
        else:  # wants to shlap another user
